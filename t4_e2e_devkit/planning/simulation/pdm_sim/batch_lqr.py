...
"""Vendored from the public navigation reference — only the imports are
rewritten (nuplan is not installed). Do not edit the numerics:
fidelity to the scoring implementation is the whole point.
"""
...
from t4_e2e_devkit.planning.simulation.pdm_sim.batch_lqr_utils import (
    _generate_profile_from_initial_condition_and_derivatives,
    get_velocity_curvature_profiles_with_derivatives_from_poses,
)
from t4_e2e_devkit.planning.simulation.pdm_sim.enums import DynamicStateIndex, StateIndex
from t4_e2e_devkit.planning.simulation.pdm_sim.geometry import normalize_angle
...
class BatchLQRTracker:
    ...
    def __init__(
        ...
        assert stopping_proportional_gain > 0, (
    ...
    def _lateral_lqr_controller(
        ...
        identity: npt.NDArray[np.float64] = np.eye(n_lateral_states, dtype=np.float64)
        ...
        states_matrix_at_step: npt.NDArray[np.float64] = np.tile(
            identity[None, None, ...], [self._tracking_horizon, batch_dim, 1, 1]
        )  # (horizon, batch, 3, 3)
