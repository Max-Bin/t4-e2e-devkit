...
from t4_e2e_devkit.planning.simulation.occupancy_map.abstract_occupancy_map import Geometry
...
class PDMOccupancyMap:
    ...
    def __init__(
        ...
        # attribute
        self._tokens = tokens
        # ``type(x) != "Polygon"`` compares a type to a string and is always
        # true, so every construction re-wrapped every polygon through
        # ``Polygon(polygon)`` -- an identity that costs a GEOS round-trip per
        # geometry, 51 maps per oracle window.  Wrap only what actually needs
        # wrapping (coordinate arrays); an existing Polygon passes through
        # unchanged, which is by definition the same value.
        if len(geometries) and not isinstance(geometries[0], Polygon):
...
class PDMDrivableMap(PDMOccupancyMap):
    ...
    def points_in_polygons(self, points: npt.NDArray[np.float64]) -> npt.NDArray[np.bool_]:
        ...
        Implemented as one bulk STR-tree query instead of one prepared
        ``contains`` sweep per polygon.  The reference sweep evaluated every
        (polygon, point) pair -- on the T4 drivable map that is 150-300
        polygons x ~13k proposal corners per scorer call, and was the single
        largest term of ``score_proposals`` (82.6 ms median, 202 ms max per
        window).  The tree prunes to the pairs whose envelopes overlap, and
        GEOS evaluates the same predicate on those: ``point.within(polygon)``
        and prepared ``contains(polygon, point)`` are the identical relate
        pattern (interior intersects interior; boundary excluded in both), so
        the boolean output is bit-identical -- pinned against the reference
        ...
        point_geometries = shapely.points(flattened_points)
        point_indices, tree_indices = self._str_tree.query(
            point_geometries, predicate="within"
        )
        output = np.zeros((len(self._geometries), len(flattened_points)), dtype=bool)
        output[tree_indices, point_indices] = True

        output_shape = (len(self._geometries),) + input_shape
        return output.reshape(output_shape)

    def _points_in_polygons_reference(
        self, points: npt.NDArray[np.float64]
    ) -> npt.NDArray[np.bool_]:
        """The original per-polygon sweep, kept as the equivalence oracle."""
        assert points.shape[-1] == 2, "Points array must have shape (...,2) for x, y coordinates!"
