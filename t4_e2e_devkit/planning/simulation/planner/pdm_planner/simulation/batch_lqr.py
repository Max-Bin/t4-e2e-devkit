...
from t4_e2e_devkit.planning.simulation.planner.pdm_planner.utils.pdm_enums import DynamicStateIndex, StateIndex
from t4_e2e_devkit.planning.simulation.planner.pdm_planner.utils.pdm_geometry_utils import normalize_angle
from t4_e2e_devkit.planning.simulation.planner.pdm_planner.simulation.batch_lqr_utils import (
...
class BatchLQRTracker:
    ...
    def _lateral_lqr_controller(
        ...
        # An elementwise expansion of these three einsums (fixed-order sums over
        # the length-3 contraction axis) was measured at ~3x fewer numpy calls,
        # but c_einsum's optimised inner product does NOT accumulate in plain
        # (t0 + t1) + t2 order: 300 random trials showed ULP-level deviations
        # up to 1e-13.  Commands feed a sequential simulation whose outputs are
        # oracle training labels, so per this repository's bit-exactness
        # contract the expansion was reverted.  Do not retry without an
        # empirical bit-equality gate.
