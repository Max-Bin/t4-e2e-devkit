...
from t4_e2e_devkit.common.actor_state.tracked_objects_types import AGENT_TYPES
from t4_e2e_devkit.common.actor_state.vehicle_parameters import VehicleParameters, get_pacifica_parameters
from t4_e2e_devkit.common.maps.maps_datatypes import SemanticMapLayer
from t4_e2e_devkit.planning.simulation.observation.collision_utils import CollisionType
from t4_e2e_devkit.planning.simulation.observation.idm_utils import is_agent_ahead, is_agent_behind
from t4_e2e_devkit.planning.simulation.trajectory.trajectory_sampling import TrajectorySampling

from t4_e2e_devkit.planning.simulation.planner.pdm_planner.observation.pdm_observation import PDMObservation
from t4_e2e_devkit.planning.simulation.planner.pdm_planner.observation.pdm_occupancy_map import PDMDrivableMap
from t4_e2e_devkit.planning.simulation.planner.pdm_planner.scoring.pdm_comfort_metrics import ego_is_comfortable
from t4_e2e_devkit.planning.simulation.planner.pdm_planner.scoring.pdm_scorer_utils import get_collision_type
...
class PDMScorer:
    ...
    def _calculate_ttc(self):
        ...
        # Queries have no side effects, so they are batched per observation
        # timestep before any state is mutated: the (time_idx, projection)
        # pairs that look at the same occupancy map -- up to four of them --
        # share one STR-tree call instead of issuing 41 x 4 = 164 small ones.
        # Results come back sorted by flat input index, so slicing them by
        # bucket reproduces each original call's pair order exactly, and the
        # mutation loop below then runs in the verbatim original
        # (time_idx, step_idx) order.  ``_calculate_ttc_reference`` keeps the
        # original one-query-per-pair implementation for the equivalence test.
        pair_results: dict = {}
        groups: dict = {}
        for time_idx in range(self.proposal_sampling.num_poses + 1):
            for step_idx, future_time_idx in enumerate(future_time_idcs):
                groups.setdefault(time_idx + int(future_time_idx), []).append(
                    (time_idx, step_idx)
                )
        for current_time_idx, pairs in groups.items():
            stacked = np.concatenate([polygons[:, t, s] for t, s in pairs])
            hits = self._observation[current_time_idx].query(
                stacked, predicate="intersects"
            )
            flat_indices, tree_indices = hits[0], hits[1]
            bucket_of = flat_indices // self._num_proposals
            proposal_of = flat_indices % self._num_proposals
            for bucket_index, pair in enumerate(pairs):
                mask = bucket_of == bucket_index
                pair_results[pair] = (proposal_of[mask], tree_indices[mask])

        # check collision for each proposal and projection
        for time_idx in range(self.proposal_sampling.num_poses + 1):
            for step_idx, future_time_idx in enumerate(future_time_idcs):
                current_time_idx = time_idx + future_time_idx
                intersecting = pair_results[(time_idx, step_idx)]
    ...
    def _calculate_ttc_reference(self):
        """The original one-query-per-(time, projection) TTC, for equivalence tests."""
