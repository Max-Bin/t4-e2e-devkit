...
multiplicative/weighted structure is copied from the public reference
implementation (file:line citations inline) and from the nuplan-devkit helpers
it imports (``is_agent_ahead/behind``,
...
need a maps DB + an old Python and conflict with our torch/numpy — verified):
  ...
  2. **Agents are GT-future, not policy-simulated.** NC/TTC use the ACTUAL
     future agent boxes from the T4 ``frames.pack`` GT fields (ground-truth future),
  ...
  4. **LK / TLC default to 1.0** (lane-keeping / traffic-light terms;
     ...
     ``val/ddc_route_frac``. **DAC is implemented** from the T4 scene's own road-border
     ...
     ``val/dac_border_frac`` alongside). The aggregate renormalises the
     weighted term over only the metrics actually computed (see
     ``aggregate_pdms``).
...
ego frame (the T4 GT layout; col3=w width, col4=l length —
...
# public reference planner scoring comfort constants, lines 18-34.
# ---------------------------------------------------------------------------
...
def comfort_score(
    ...
    dt: float,
    *,
    initial_velocity: "npt.NDArray[np.float64] | None" = None,
    wheel_base: float = 2.79,
    simulate: bool = True,
) -> npt.NDArray[np.float64]:
    """See ``_comfort_score_raw``; by default this SIMULATES first.

    navsim does not score a planner's raw waypoints: ``pdm_scorer`` calls
    ``ego_is_comfortable`` on ``PDMSimulator.simulate_proposals`` output — the
    plan after LQR tracking through a kinematic bicycle model, whose reference
    is a jerk-penalised least-squares fit. Differentiating raw waypoints makes
    ...
    if simulate:
        ...
        flat = p.reshape(-1, T, 4)
        heading = np.arctan2(flat[..., 3], flat[..., 2])
        xyh = np.stack([flat[..., 0], flat[..., 1], heading], axis=-1)
        if initial_velocity is None:
            v0 = np.linalg.norm(xyh[:, 1, :2] - xyh[:, 0, :2], axis=-1) / dt
        else:
            v0 = np.asarray(initial_velocity, dtype=np.float64).reshape(-1)
        states = simulate_proposals(xyh, v0, wheel_base=wheel_base, dt=dt)
        ok = ego_is_comfortable(states, np.arange(T).astype(np.float64) * dt).all(axis=-1)
        return np.asarray(ok.astype(np.float64).reshape(lead), dtype=np.float64)
    return _comfort_score_raw(poses, dt)


def _comfort_score_raw(poses: npt.NDArray[np.float64], dt: float) -> npt.NDArray[np.float64]:
...
def box_corners(box: npt.NDArray[np.float64]) -> npt.NDArray[np.float64]:
    ...
    Velocity caveat: cols (7,8) are tracker-smoothed and unreliable in
    MAGNITUDE for MOVING agents (measured ~0.5x true speed, 2026-07-08 audit);
    only the ~0 static signal is trustworthy.
...
def is_track_stopped(
    ...
    NB: T4 GT velocity magnitudes are tracker-smoothed and unreliable for
    MOVING agents (measured ~0.5x true speed, 2026-07-08 audit); only the ~0
    static signal consumed here is load-bearing.
    """
...
def aggregate_pdms(
    *,
    ...
    """PDMS = prod(multiplicative) * weighted_avg(weighted metrics).
    ...
    reference reports "unavailable" — e.g. the first plan of a scene); the
    denominator renormalises over only the provided weights (deviation #4 —
    DAC/DDC/LK/TLC default to 1.0 / EP|TTC|comfort are passed in). With the
    navsim four present and the multiplicative metrics at 1.0, this reproduces
    navsim's ``_aggregate_pdm_scores`` exactly.
    """
...
def dac_from_road_borders(
    ...
    polygons; the T4 bundle carries the lanelet maps' ``road_border`` line strings
...
def ddc_from_route_lanes(
    ...
    are built from the T4 scene's ROUTE tensor (centerline ± boundary-offset
...
def ego_area_flags(
    ...
    - non_drivable: at least one corner outside EVERY drivable polygon
      (drivable = lanes + intersections here — the T4 bundle carries no carpark
...
def lane_keeping_score(
    ...
    - exemptions: inside an intersection; queue (speed <= 1 m/s AND <= 1.5 m
      ...
      indicator windows;
    - LK = 1 iff no continuous violation run reaches 2 s. A run's duration is
      measured first-to-last sample = (N-1)*dt (Autoware
      lane_keeping.cpp:116-119), so at dt=0.1 the first FAILING run is 21
      consecutive over-threshold samples (2.0 s), not 20 (1.9 s); else 0.
    ...
    to the whole horizon. Route centerlines are the T4 window's 25 route
    ...
    for t in range(T):
        ...
        # Autoware measures a run's duration first-to-last sample = (N-1)*dt
        # (lane_keeping.cpp:116-119): a run of exactly N samples fails only when
        # (N-1)*dt >= 2 s -> first failure at N=21 (dt=0.1), NOT N=20 (run*dt).
        if (run - 1) * dt >= LK_MAX_VIOLATION_S:
            return 0.0
    return 1.0
...
# --- navsim's OWN extended comfort (preferred over the Autoware C++ port) ----
# run_pdm_score.py:335-368 + pdm_comfort_metrics.py:437-495. navsim compares
# two CONSECUTIVE plans on their OVERLAPPING horizon, using the same
# ``ego_simulated_states`` it scores comfort on, and thresholds the RMS of the
# per-feature differences. Same four thresholds as the C++ port below (0.7 /
# 0.5 / 0.1 / 0.1), but the features come from simulated state FIELDS, not
# from differentiating raw waypoints.
EC_NS_ACCEL, EC_NS_JERK, EC_NS_YAW_RATE, EC_NS_YAW_ACCEL = 0.7, 0.5, 0.1, 0.1


def _ec_features(
    states: npt.NDArray[np.float64], time_point_s: npt.NDArray[np.float64], wheel_base: float
) -> dict:
    """pdm_comfort_metrics.py::extract_features on simulated states."""
    # our ported helpers take no vehicle_parameters (they read the state
    # fields directly, which is all navsim's magnitude mode uses)
    del wheel_base
    return {
...
def extended_comfort_navsim(
    ...
    """navsim two-frame extended comfort for a pair of consecutive plans.

    Both plans are SIMULATED first (LQR + bicycle model), then compared on the
    overlap: ``prev[k:]`` vs ``curr[:-k]`` with ``k = observation_interval/dt``
    (run_pdm_score.py:341-352). Score is 1 iff the RMS of every feature
    difference is within threshold. Raw-pose differentiation (the C++-derived
    ...
    def sim(poses4, v0):
        h = np.arctan2(poses4[:, 3], poses4[:, 2])
        xyh = np.stack([poses4[:, 0], poses4[:, 1], h], axis=-1)[None]
        if v0 is None:
            v0 = float(np.linalg.norm(xyh[0, 1, :2] - xyh[0, 0, :2]) / dt)
        return simulate_proposals(xyh, np.array([v0]), wheel_base=wheel_base, dt=dt)[0]

    v_prev, v_curr = initial_velocity or (None, None)
    s_prev, s_curr = (
        sim(np.asarray(prev_poses, float), v_prev),
        sim(np.asarray(curr_poses, float), v_curr),
    )
    a, b = s_prev[k:], s_curr[:-k]
    t = np.arange(a.shape[0]).astype(np.float64) * dt
    fa, fb = _ec_features(a[None], t, wheel_base), _ec_features(b[None], t, wheel_base)
    ok = True
    for key, thr in (
        ("acceleration", EC_NS_ACCEL),
        ("jerk", EC_NS_JERK),
        ("yaw_rate", EC_NS_YAW_RATE),
        ("yaw_accel", EC_NS_YAW_ACCEL),
    ):
        rms = float(np.sqrt(np.mean((fa[key] - fb[key]) ** 2)))
        ok = ok and rms <= thr
    return float(ok)
